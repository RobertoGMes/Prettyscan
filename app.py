...
function risk(p){
  const t=T[lang];
  const c=[21,22,23,3389,5900,6379,27017,9200];
  const h=[25,53,80,110,143,443,445,3306,5432,8080,8443];
  const n=parseInt(p);
  ...
  d.hosts.forEach((h,i)=>{
    ...
    if(h.ports&&h.ports.length){
      pt=`<div class="pw"><table class="pt">
        <thead><tr><th>${t.thPort}</th><th>${t.thProto}</th><th>${t.thState}</th><th>${t.thSvc}</th><th>${t.thVer}</th><th>${t.thRisk}</th></tr></thead>
    ...
    div.innerHTML=`<div class="hh"><div><div class="hip">${h.ip}</div>${hn}</div><div class="hi">${stTag}${os}<span style="color:var(--mt);font-size:.8rem">${h.ports?h.ports.length:0} <span class="txt-ports">${t.ports}</span></span>${actions}</div></div>${pt}`;
    hc.appendChild(div);
  });
    ...
    for h in root.findall('host'):
        ...
        if pe is not None:
            for p in pe.findall('port'):
                ...
                ports.append({'port': pid, 'protocol': proto, 'state': pstate, 'service': service, 'version': version})
        hosts.append({'ip': ip, 'hostname': hostname, 'state': state, 'os': os_name, 'ports': ports})
    return hosts
